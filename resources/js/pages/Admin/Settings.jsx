...
const AdminSettings = ({ auth, pixelSettings }) => {
    ...
    const handleSubmit = (e) => {
        ...
        put(route('admin.settings.pixel.update'), {
            onSuccess: () => toast.success(t('admin.pixel_settings_updated', 'Paramètres des pixels mis à jour.')),
        });
    };

    return (
        <AdminLayout user={auth.user}>
            <Head title={t('admin.pixel_settings', 'Paramètres des Pixels')} />
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">{t('admin.tracking_pixels', 'Pixels de Tracking')}</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('admin.manage_pixels_desc', 'Gérez plusieurs IDs pour Meta, Google, TikTok et Snapchat.')}</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {(data.pixels || []).map((pixel, index) => (
                            ...
                            >
                                <div className="w-full md:w-1/4 space-y-2">
                                    <label htmlFor={`platform-${index}`} className="text-xs font-bold uppercase text-gray-400">{t('admin.platform', 'Plateforme')}</label>
                                    <select
                                        id={`platform-${index}`}
                                ...
                                <div className="w-full md:flex-1 space-y-2">
                                    <label htmlFor={`pixel-id-${index}`} className="text-xs font-bold uppercase text-gray-400">{t('admin.pixel_id', 'ID du Pixel')}</label>
                                    <input
                                        id={`pixel-id-${index}`}
                                        name={`pixels[${index}][pixel_id]`}
                                        type="text"
                                        placeholder={t('admin.pixel_id', 'ID du Pixel')}
                                ...
                                <div className="w-full md:w-1/5 space-y-2">
                                    <label htmlFor={`name-${index}`} className="text-xs font-bold uppercase text-gray-400">{t('admin.name_optional', 'Nom (Optionnel)')}</label>
                                    <input
                                        id={`name-${index}`}
                                        name={`pixels[${index}][name]`}
                                        type="text"
                                        placeholder={t('admin.label', 'Label')}
                                        value={pixel.name}
                                ...
                                <div className="flex gap-2 items-center h-10">
                                    <button
                                        ...
                                        title={pixel.is_active ? t('admin.active', 'Actif') : t('admin.inactive', 'Inactif')}
                                    >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        >
                            ...
                            {t('admin.add_pixel', 'Ajouter un Pixel')}
                        </Button>
